...
import { deleteMilestone, updateMilestone } from '../../../../../../lib/server/client-admin';
...
export const PUT: APIRoute = async ({ request, cookies, params }) => {
  ...
  if (!entryId) return badRequestResponse('Missing milestone id.');
  ...
  if (body.title !== undefined) {
    ...
    if (!title) return badRequestResponse('Milestone title cannot be empty.');
    input.title = title;
  }
  ...
  if (body.milestone_date !== undefined) input.milestone_date = cleanNullableString(body.milestone_date);
  if (body.sort_order !== undefined) input.sort_order = cleanNumber(body.sort_order);

  const result = await updateMilestone(entryId, input);
...
export const DELETE: APIRoute = async ({ cookies, params }) => {
  ...
  const result = await deleteMilestone(entryId);
  if ('error' in result) return serverErrorResponse(result.error);
